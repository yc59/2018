...
Given a set of observations of fish lengths spanning three age classes, estimate the parameters of the Gaussian (normal) distributions of length-at-age, when the age of each length observation is not known. 
...
If we knew the distribution component that generated each observation, we would use maximum likelihood to estimate mean length $\hat{\mu}_k$ and standard deviation $\hat{\sigma}_k$ of each of the $k$ age classes  We would also know the proportion of observations coming from each age class $\lambda_k$.  Because the generator of each observation is not know (it is hidden or latent), the key to the estimation problem is to **assign** labels (age classes) to observations by some process, and subsequently perform maximum likelihood estimation of $\hat{\mu}_k$ and $\hat{\sigma}_k$. This assignment and estimation is performed repeatedly until the parameter estimates cease changing.

# Algorithm to perform the estimation
The expectation maximization (EM) algorithm is the tool we will use for this task.  It consists of the following steps: initialization, iteration over expectation, and maximization, with test for convergence determining when iteration can stop.
...
## Initialization
1. Using data from the fish of known age as a starting point, assign labels (age classes) to each of $N$ length observations in the dataset that come from fish of unknown age.
...
This *posterior probability* is computed for each observation $x_i$ based upon our initial guesses about the model parameters.  $P(\cdot)$ represents the probability of the event described inside the parenthesis.
...
note $\mathcal{N}()$ refers to the normal distribution and $\sigma_k^2$ refers to the variance (rather than standard deviation) of the $k^{th}$ component of the mixture; not the estimate of the standard deviation from the second iteration of the EM algorithm.
...
You will be working in teams of four.  It will be your responsibility to design your code (how many functions, how is information passed between functions) and delegate code writing, documentation, and testing duties among team members.  We suggest implementing the algorithm can be conveniently broken into three or four functions.  While we have asked you to apply the EM algorithm to a specific dataset, your code should be generalizeable to other datasets.  The README file of your repository will serve as your project report, and should include the text, figures, and tables described in the tasks below.
...
- Include a diagram of your team's approach to the problem.  This can be digital or hand-drawn and scanned in as a PDF. 

## Task 3: Implement the algorithm 

Create modular code consisting of a set of documented and tested functions implementing this algorithm.  Your main function, called by the user, will be called `teamEM()` and it will take three arguments: 
...
- Write a function to create a simulated dataset with similar properties to the "true" data

- Show that the algorithm returns correct values for the simulated dataset
...
This assignment will be marked with computer assistance, just like Assignment 1.  The team's code will be sourced, arguments `data`, `epsilon` and `maxit` the primary (wrapper function) named `teamEM()`.  The output from the function will be checked against results of Gaussian mixture model fitting produced by the function `normalmixEM2comp()` in the package `mixtools`.  Note when you use the `normalmixEM2comp()` function, the argument `sigsqrd` is an initial value of the two *variances* rather than standard deviations.

For example, testing code would evaluate operation of your submission like this:

```
mod <- teamEM(data, epsilon = 1e-08, maxit = 1000) 
mixmod <- normalmixEM2comp(data, mu = mod$inits$mu, 
...
* ten marks will be awarded based on the automated tests, 
* ten marks for evidence of code design and testing,
* ten marks for programming style,
* ten marks for documentation,
* ten marks for any enhancements your team might wish to add to increase functionality of the code, produce additional figures, or to improve documentation.

All members of the team will receive the same mark.  

Because this assignment counts towards your final grade, it is important that you do not collaborate with individuals outside your team in completing the work.   You should be comfortable with the following statement, which you should include as a comment at the beginning of the `teamEM()` file:

`# I confirm that the attached is the work of our team, except where clearly indicated in the text.`
