...
This assignment will be marked with computer assistance, just like Assignment 1.  The team's code will be sourced, the output from the `teamEM` function will be checked against results of Gaussian mixture model fitting produced by the function `normalmixEM2comp` in the package `mixtools`.  Note when you use the `normalmixEM2comp` function, the argument `sigsqrd` is an initial value of the two *variances* rather than standard deviations.
